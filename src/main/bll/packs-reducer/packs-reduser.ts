...
export const packsReduser = (state: InitialPacksStateType = initialState, action: PacksActionType): InitialPacksStateType => {
    switch (action.type) {
...
export const setActivModalAC = (active: boolean) => ({ type: "DIMA/TEAM-PROJECT/CARDS/SET-ACTIVE-MODAL", active } as const);
export const setCurrentPageAC = (value: number) => ({ type: "DIMA/TEAM-PROJECT/CARDS/SET-CARENT-PAGE", value } as const);
export const setPackCardsIdAC = (packid: number) => ({ type: "DIMA/TEAM-PROJECT/CARDS/SET-PACK-CARDS-ID", packid } as const);
